...
$it.name.literal$$if(it.orcid)$~\orcidlink{$it.orcid$}$endif$$for(it.affiliations)$\\$it.name$\\$it.city$, $it.country$$endfor$\\$if(it.email)$Corresponding author: \href{mailto:$it.email$}{$it.email$}$endif$